...
public class Simulator {
	...
	public void simulate() {
		...
		while (clock < steps) {
			...
			try {
				s = incomingQueue.peek();

				if (lot.attemptParking(s.getCar(), clock)) {
					incomingQueue.dequeue();
					lot.park(s.getCar(), clock);
					/*
					System.out.println(s.getCar() + " ENTERED at timestep " + clock
								+ "; occupancy is at " + lot.getCapacity());
					*/
				}
			...
			try {
				s = outgoingQueue.dequeue();
				/*
				System.out.println(s.getCar() + " EXITED at timestep " + clock + "; occupancy is at "
						+ lot.getCapacity());
				*/
			} catch (IllegalStateException e) {
				// do nothing
